...
theta.0 = runif(1, l_min, l_max)
# choosing the starting point of f to be y/2
f.0 = training_data[, 2]/2
...
l <- function(x, mu = rep(0, length(x)), sigma = diag(1, length(x)), tau = 0.1){
  return(mvtnorm::dmvnorm(x, mu, tau * sigma))
}
SurrogateMH(theta = theta.0, f = f.0, data = training_data, p = prior, l)
```
...
### ```SurrogateSS```
This function updates the hyperparameter and the latent variable according to the slice sampling algorithm provided in Adams and Murray(2010). 
```{r}
# set.seed(12345)
f = vector("list", length = 1 + 1)
th = vector("numeric", 1 + 1)

## choosing the starting points
f[[1]] = training_data[, 1]/2
th[1] = runif(1, l_min, l_max)

## choosing the scale parameter
sigma = 0.2
i = 1
SurrogateSS(theta = th[i], f = f[[i]], sigma = sigma, data = training_data, p = prior, niter = 100)
#   f[[i + 1]] = result$f
#   th[i + 1] = result$theta
# f[[2]]
# th[2]
